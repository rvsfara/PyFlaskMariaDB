...
def frm_entry():
    ...
    cur.execute("insert into tbCrudFlask (value_1,value_2,operator,result, remark) values (%s,%s,%s,%s,%s)",((xvalue_1,xvalue_2,xoperator,resultnya,rem)))
...
def display_data():
    ...
    cur=mysqlnya.connect().cursor()
    cur.execute("select * from tbCrudFlask")
...
def frm_edit_data(id):
    cur=mysqlnya.connect().cursor()
    form=EntryForm()
    cur.execute("select * from tbCrudFlask where id='"+id+"'")
    hasilnya=cur.fetchall()    
    ...
    if request.method=="POST":       
       ...
       cur=mysqlnya.connect().cursor()
       #cur.execute("update tbCrudFlask set value_1='"+xnilai_1+"'"+
       #            ",value_2='"+xnilai_2+"'"+",operator='"+xoperator+"'"+
       #            ",result='"+hasil+"'"+",remark='"+xremark+"'"+"where id='"+id+"'")
       
       cur.execute ("update tbCrudFlask set value_1=%s,value_2=%s,operator=%s,result=%s,remark=%s where id= %s",
...
def delete_data(id):
    cur=mysqlnya.connect().cursor()
    cur.execute("delete from tbCrudFlask where id='"+id+"'")
    cur.connection.commit()
    cur.close()    
